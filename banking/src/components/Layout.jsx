    ...
    <div className="flex min-h-screen bg-gray-50 font-sans text-gray-900">
        ...
        <nav className="flex flex-1 flex-col gap-1">
          {nav.map((n) => (
            <NavLink
              ...
              to={n.to}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium ${isActive ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50'}`
              }
            >
      ...
      <div className="flex-1">
        <header className="flex items-center justify-between border-b border-gray-100 bg-white px-6 py-4 sm:hidden">
          <span className="font-bold text-brand-700">Meridian</span>
